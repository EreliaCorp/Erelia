...
Message encounter_modification_message(Server_message::Encounter_modification);
...
void Player_interacter::_compose_pos(jgl::Vector2Int& p_actual, jgl::Vector2Int& p_start, jgl::Vector2Int& p_end)
{
	jgl::Vector2Int tmp_second_pos = convert_screen_to_world(jgl::Application::active_application()->mouse().pos());
...
void Player_interacter::place_multiple_area_value(jgl::Vector2Int p_start, jgl::Vector2Int p_end, jgl::Int p_value)
{
	encounter_modification_message.clear();
	...
	for (jgl::Int x = p_start.x; x <= p_end.x; x++)
	{
		for (jgl::Int y = p_start.y; y <= p_end.y; y++)
		{
			...
			if (Engine::instance()->map()->encounter(tmp_pos) != p_value)
			{
				...
				encounter_modification_message << tmp_pos << p_value;
			}
		}
	}

	if (encounter_modification_message.empty() == false)
		Client_manager::client()->send(encounter_modification_message);
}

void Player_interacter::place_single_area_value(jgl::Vector2Int p_pos, jgl::Int p_value)
{
	encounter_modification_message.clear();
	...
	if (Engine::instance()->map()->encounter(p_pos) != p_value)
	{
		...
		encounter_modification_message << p_pos << p_value;
	}
...
void Player_interacter::paint_area_value(jgl::Vector2Int p_pos, jgl::Int p_value)
{
	encounter_modification_message.clear();
	...
	for (jgl::Size_t i = 0; i < pos_array.size(); i++)
	{
		...
		encounter_modification_message << pos_array[i] << p_value;
	}
...
void Player_interacter::place_circle_area_value(jgl::Vector2Int p_pos, jgl::Float p_radius, jgl::Int p_value)
{
	encounter_modification_message.clear();
...
jgl::Bool Player_interacter::_builder_update()
{
...
jgl::Bool Player_interacter::_adventure_update()
{
...
jgl::Bool Player_interacter::_monster_area_update()
{
	jgl::Input_status left_button = jgl::Application::active_application()->mouse().get_button(jgl::Mouse_button::Left);
	jgl::Input_status right_button = jgl::Application::active_application()->mouse().get_button(jgl::Mouse_button::Right);
	jgl::Input_status shift_key = jgl::Application::active_application()->keyboard().get_key(jgl::Key::Shift);
	...
	if (shift_key == jgl::Input_status::Down)
	{
		if (left_button == jgl::Input_status::Release)
		{
			...
			Map::send_area_edition_command(p_start, p_end, _monster_value);
		...
		else if (right_button == jgl::Input_status::Release)
		{
			...
			Map::send_area_edition_command(p_start, p_end, -1);
		}
	}
	else if (shift_key == jgl::Input_status::Up)
	{
		if (left_button == jgl::Input_status::Down)
		{
			...
			if (jgl::Application::active_application()->keyboard().get_key(jgl::Key::Alt) == jgl::Input_status::Up)
				place_single_area_value(p_actual, _monster_value);
			else
...
jgl::Bool Player_interacter::_update()
{
	...
	if (Game_screen::instance()->gamemode() == Gamemode::Adventure)
		return (_adventure_update());
	else if (Game_screen::instance()->gamemode() == Gamemode::Builder)
		return (_builder_update());
	else if (Game_screen::instance()->gamemode() == Gamemode::Area_builder)
		return (_monster_area_update());
