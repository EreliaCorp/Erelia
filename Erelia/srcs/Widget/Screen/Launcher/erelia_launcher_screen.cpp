...
void Launcher_screen::_send_connection()
{
	static Message msg(Server_message::Connection_request);
	
	msg.clear();
	
	msg << _username_input.entry->label().text();
	msg << _password_input.entry->label().text();

	Client_manager::client()->send(msg);
}

void Launcher_screen::_send_register()
{
...
Launcher_screen::Launcher_screen(jgl::Widget* p_parent) : Abstract_screen(p_parent)
{
	...
	Publisher::instance()->subscribe(Launcher_screen::Event::Connection_complete, LAUNCHER_SCREEN_ACTIVITY_PARAM{
			_connect_button->define_function([&](jgl::Data_contener& p_param) {
					if (_username_input.entry->label().text().size() >= 4 && _password_input.entry->label().text().size() >= 4)
						_send_connection();
				});
			_register_button->define_function([&](jgl::Data_contener& p_param) {
					if (_username_input.entry->label().text().size() >= 4 && _password_input.entry->label().text().size() >= 4)
						_send_register();
				});
